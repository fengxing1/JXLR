...
- (void)initView
{
    ...
    _listView = [[UIFolderTableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-HeightNavigationAndStateBar) style:UITableViewStyleGrouped];
...
- (void)expandBtnClick:(UIButton *)btn
{
    ...
    if (expandClicked) {
        ...
        Collection *collection = _dataArrays[btn.tag];
         ...
         if (collection.transferStatus == -1 || collection.transferStatus == 1 || collection.transfersId != 0) {
...
//            collection.bidAmount = [[item objectForKey:@"bid_amount"] floatValue];
            collection.transfersId = [[item objectForKey:@"transfers_id"] intValue];
