...
- (void)initView
{
    ...
    _leftBtn.frame = CGRectMake(5.0f, SpaceMediumSmall, 50.0f, 40.0f);
    _leftBtn.backgroundColor = ColorBGGray;
    _leftBtn.tag = 1;
    ...
    _rightBtn.frame = CGRectMake(self.view.frame.size.width - 55.0f, SpaceMediumSmall, 50.0f, 40.0f);
    _rightBtn.backgroundColor = ColorBGGray;
    _rightBtn.tag = 2;
    ...
    // 中间部分 年月
    _yearlabel = [[UILabel alloc] initWithFrame:CGRectMake((self.view.frame.size.width - 150.0f) * 0.5, SpaceBig, 150.0f, 30.0f)];
    ...
    // 画直线
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_rightBtn.frame)+SpaceSmall/2, self.view.frame.size.width, HeightLine)];
    lineView.backgroundColor = ColorLine;
    [backView addSubview:lineView];
    
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(lineView.frame), self.view.frame.size.width, 250)];
    ...
    backView.frame=CGRectMake(0, SpaceSmall/2, WidthScreen, CGRectGetMaxY(_scrollView.frame));
}
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        for (NSDictionary *item in collections) {
            ...
            int index=curr + 12 - monthnum - 1;
            if(_collectionArrays.count>index)
