...
- (void)initView
{
    ...
    //选择框
    _ComboBox = [[AJComboBox alloc] initWithFrame:CGRectMake(5, 8, 90, 20)];
    _ComboBox.table.frame= CGRectMake(_ComboBox.frame.origin.x, _ComboBox.frame.origin.y+84, _ComboBox.frame.size.width, [_dataArr count]*30);
    ...
    _listView = [[UIFolderTableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(backView.frame), self.view.frame.size.width, self.view.frame.size.height - CGRectGetMaxY(backView.frame)-HeightNavigationAndStateBar) style:UITableViewStyleGrouped];
    ...
    // 时间选择器背景View
    _timeView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height, WidthScreen, 260)];
...
- (void)ViewAnimation:(UIView*)view willHidden:(BOOL)hidden {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        if (hidden) {
            _backView.alpha = 0;
            _listView.scrollEnabled = YES;
            view.frame = CGRectMake(0, self.view.frame.size.height, WidthScreen, 260);
        } else {
            _backView.alpha = 0.5f;
            _listView.scrollEnabled = NO;
            [view setHidden:hidden];
            view.frame = CGRectMake(0, self.view.frame.size.height - 220, WidthScreen, 260);
        }
