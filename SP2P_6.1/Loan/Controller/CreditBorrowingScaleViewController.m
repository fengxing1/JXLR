...
- (void)initView
{
    ...
    _listView.backgroundColor = ColorBGGray;
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        CGSize labelSize = [reviewStr boundingRectWithSize:CGSizeMake(WidthScreen-92, 2000) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
        ...
        _promptLabel.frame = CGRectMake(8, CGRectGetMaxY(label33.frame)+10,WidthScreen-10, 40);
        ...
        //NSLog(@"返回成功===========%@",[obj objectForKey:@"msg"]);
...
- (void)initNavigationBar
{
    ...
    // 发布借款
    BarButtonItem *barButtonPublish=[BarButtonItem barItemWithTitle:@"发布借款" widthImage:[UIImage imageNamed:@"bar_right"] selectedImage:[UIImage imageNamed:@"bar_right_press"] withIsImageLeft:NO target:self action:@selector(publishClick)];
    [self.navigationItem setRightBarButtonItem:barButtonPublish];
}
...
- (void)publishClick
{
    ...
    if (AppDelegateInstance.userInfo == nil) {
        ...
        }else if (_isBaseInfo == 0 )//已激活未完善资料
        {
