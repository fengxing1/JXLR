...
-(void)onOpen{
    ...
    controller.openAccountType=OpenAccountTypeDefault;
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([error isEqualToString:@"-1"]) {
        ...
        //NSLog(@"返回成功  msg -> %@",[obj objectForKey:@"msg"]);
        
        if (isOPT == 151) {
            //NSLog(@"VIP初始化返回 %@", dics);
            
            vipFee = [[obj objectForKey:@"vipFee"] intValue];
            vipMinTimeLength = [[obj objectForKey:@"vipMinTimeLength"] intValue];
    ...
    else if ([error isEqualToString:@"-999"])
    {
        ...
        //NSLog(@"返回失败  msg -> %@",[obj objectForKey:@"msg"]);
