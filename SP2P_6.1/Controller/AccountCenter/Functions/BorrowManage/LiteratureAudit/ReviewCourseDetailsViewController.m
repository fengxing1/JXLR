...
- (void)initView
{
    ...
    _ScrollView.contentInset=UIEdgeInsetsMake(0, 0, 44, 0);
    ...
    for (int i = 0; i < [titleArr count]; i++) {
        ...
        if (i == titleArr.count - 1){
            titleLabel.frame = CGRectMake(15, SpaceMediumSmall + i * 35, 120, 30);
        }else {
            titleLabel.frame = CGRectMake(15, SpaceMediumSmall + i * 35, 100, 30);
        }
    ...
    if (_literatureAudit!= nil &&  _literatureAudit.status !=2 &&  _literatureAudit.status !=1) {
        ...
        _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 44-HeightNavigationAndStateBar, self.view.frame.size.width, 44)];
