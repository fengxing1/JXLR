...
- (void)initView
{
    ...
    _scrollView.contentInset=UIEdgeInsetsMake(0, 0, 90, 0);
    ...
    UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 90-HeightNavigationAndStateBar, self.view.frame.size.width, 90)];
    ...
    if(!_isPay)
    {
        ...
        UIButton *repayBtn = [ToolButton CreateMainButton:CGRectMake(SpaceBig, bottomView.frame.size.height-40,WidthScreen-SpaceBig*2, 30) withTitle:@"还  款" withTarget:self withAction:@selector(repayBtnClick)];
        [bottomView addSubview:repayBtn];
    }else{
        [bottomView setHidden:YES];
        _scrollView.contentInset=UIEdgeInsetsMake(0, 0, SpaceBig, 0);
    }
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    }else if(_requestType == 1) // 还款
    {
        ...
        else if ([error isEqualToString:@"-999"]||[error isEqualToString:@"(null)"]) {
