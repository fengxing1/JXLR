...
@property (nonatomic,strong) UIView *viewMessageBG;//信息背景
...
- (void)initView
{
    ...
    _viewMessageBG=[[UIView alloc] init];
    _viewMessageBG.backgroundColor=[UIColor whiteColor];
    [self.view addSubview:_viewMessageBG];
    // 标题
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, SpaceMediumSmall, 150, 30)];
    _titleLabel.font = [UIFont boldSystemFontOfSize:14.0f];
    _titleLabel.textColor = [UIColor blackColor];
    [_viewMessageBG addSubview:_titleLabel];
    
    // 时间
    _timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(170, SpaceMediumSmall, 150, 30)];
    _timeLabel.font = [UIFont systemFontOfSize:14.0f];
    _timeLabel.textColor = [UIColor darkGrayColor];
    [_viewMessageBG addSubview:_timeLabel];
    
    UILabel *line = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_timeLabel.frame), self.view.frame.size.width + 100, 2)];
    line.text = @"-----------------------------------------------------------------------";
    line.textColor = ColorLine;
    [_viewMessageBG addSubview:line];
    
    _contentLabel = [[UILabel alloc] initWithFrame:CGRectZero];
    _contentLabel.font = [UIFont systemFontOfSize:14.0f];
    _contentLabel.numberOfLines = 0;
    [_viewMessageBG addSubview:_contentLabel];
    
    //  底部模块及一些按钮功能
    UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - HeightNavigationAndStateBar-40, self.view.frame.size.width, 40)];
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        if (statusOPT == 136) {
            ...
            _contentLabel.frame = CGRectMake(10, CGRectGetMaxY(_timeLabel.frame)+SpaceMediumSmall, self.view.frame.size.width - 20, _label1Sz.height + 5);
            _viewMessageBG.frame=CGRectMake(0, SpaceSmall/2, WidthScreen, CGRectGetMaxY(_contentLabel.frame)+SpaceMediumSmall);
