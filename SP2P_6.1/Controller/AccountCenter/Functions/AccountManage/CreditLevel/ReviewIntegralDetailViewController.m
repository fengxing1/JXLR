...
- (void)initView
{
    ...
    _tableView = [[UIFolderTableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-HeightNavigationAndStateBar) style:UITableViewStyleGrouped];
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        for (NSDictionary *item in dataArr) {
            ...
            bean.auditFee = [[item objectForKey:@"audit_fee"] intValue]; //
