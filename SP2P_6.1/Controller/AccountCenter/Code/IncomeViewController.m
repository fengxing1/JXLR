...
- (void)initContentView
{
    ...
    if (_isLogin) {
        ...
        UIView *whiteView = [[UIView alloc] initWithFrame:CGRectMake(0, SpaceSmall/2, self.view.frame.size.width, 225)];
        whiteView.backgroundColor = [UIColor whiteColor];
        [self.view addSubview:whiteView];
        
        //  左边按钮
        _leftBut = [UIButton buttonWithType:UIButtonTypeCustom];
        _leftBut.frame = CGRectMake(5.0f, SpaceMediumSmall, 50.0f, 40.0f);
        _leftBut.backgroundColor = ColorBGGray;
        _leftBut.tag = 1;
        ...
        [whiteView addSubview:_leftBut];
        
        //  右边按钮
        _rightBut = [UIButton buttonWithType:UIButtonTypeCustom];
        _rightBut.frame = CGRectMake(self.view.frame.size.width - 55.0f, SpaceMediumSmall, 50.0f, 40.0f);
        _rightBut.backgroundColor = ColorBGGray;
        _rightBut.tag = 2;
        ...
        [whiteView addSubview:_rightBut];
        
        // 中间部分 年月
        _dateTime = [[UILabel alloc] initWithFrame:CGRectMake(0, SpaceBig, WidthScreen, 30.0f)];
        ...
        [whiteView addSubview:_dateTime];
        
        // 画直线
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_rightBut.frame)+SpaceMediumSmall, self.view.frame.size.width, HeightLine)];
        lineView.backgroundColor = ColorLine;
        [whiteView addSubview:lineView];
        
        _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(lineView.frame), self.view.frame.size.width, 155)];
...
//        _scrollView.delegate = self;
        ...
        DetailBut.frame = CGRectMake((self.view.frame.size.width - 150)*0.5,CGRectGetMaxY(whiteView.frame)+SpaceBig, 150.0f, 30.0f);
...
-(void) httpResponseSuccess:(NetWorkClient *)client dataTask:(NSURLSessionDataTask *)task didSuccessWithObject:(id)obj
{
    ...
    if ([[NSString stringWithFormat:@"%@",[dics objectForKey:@"error"]] isEqualToString:@"-1"]) {
        ...
        for (NSDictionary *item in collections) {
            ...
            NSInteger index = (curr + 12 - monthnum - 1) % _collectionArrays.count;
            
            [_collectionArrays replaceObjectAtIndex:index withObject:bean];
